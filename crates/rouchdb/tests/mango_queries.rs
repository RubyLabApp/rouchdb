...
async fn mango_nested_field_query() {
    ...
    remote.put("a", serde_json::json!({"address": {"city": "NYC", "state": "NY"}})).await.unwrap();
    remote.put("b", serde_json::json!({"address": {"city": "LA", "state": "CA"}})).await.unwrap();
    remote.put("c", serde_json::json!({"address": {"city": "SF", "state": "CA"}})).await.unwrap();
